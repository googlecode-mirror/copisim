<?php

/**
 * BaseCopisimItem
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $periode
 * @property string $type
 * @property string $titre
 * @property CopisimPeriode $CopisimPeriode
 * @property Doctrine_Collection $CopisimEtudiant
 * @property Doctrine_Collection $CopisimPoste
 * @property Doctrine_Collection $CopisimChoix
 * @property Doctrine_Collection $CopisimFlux
 * 
 * @method integer             getPeriode()         Returns the current record's "periode" value
 * @method string              getType()            Returns the current record's "type" value
 * @method string              getTitre()           Returns the current record's "titre" value
 * @method CopisimPeriode      getCopisimPeriode()  Returns the current record's "CopisimPeriode" value
 * @method Doctrine_Collection getCopisimEtudiant() Returns the current record's "CopisimEtudiant" collection
 * @method Doctrine_Collection getCopisimPoste()    Returns the current record's "CopisimPoste" collection
 * @method Doctrine_Collection getCopisimChoix()    Returns the current record's "CopisimChoix" collection
 * @method Doctrine_Collection getCopisimFlux()     Returns the current record's "CopisimFlux" collection
 * @method CopisimItem         setPeriode()         Sets the current record's "periode" value
 * @method CopisimItem         setType()            Sets the current record's "type" value
 * @method CopisimItem         setTitre()           Sets the current record's "titre" value
 * @method CopisimItem         setCopisimPeriode()  Sets the current record's "CopisimPeriode" value
 * @method CopisimItem         setCopisimEtudiant() Sets the current record's "CopisimEtudiant" collection
 * @method CopisimItem         setCopisimPoste()    Sets the current record's "CopisimPoste" collection
 * @method CopisimItem         setCopisimChoix()    Sets the current record's "CopisimChoix" collection
 * @method CopisimItem         setCopisimFlux()     Sets the current record's "CopisimFlux" collection
 * 
 * @package    copisim
 * @subpackage model
 * @author     Pierre-FrançoisPilouAngrand
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BaseCopisimItem extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('copisim_item');
        $this->hasColumn('periode', 'integer', null, array(
             'type' => 'integer',
             'notnull' => true,
             ));
        $this->hasColumn('type', 'string', 20, array(
             'type' => 'string',
             'notnull' => true,
             'length' => 20,
             ));
        $this->hasColumn('titre', 'string', 255, array(
             'type' => 'string',
             'notnull' => true,
             'length' => 255,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('CopisimPeriode', array(
             'local' => 'periode',
             'foreign' => 'id'));

        $this->hasMany('CopisimEtudiant', array(
             'local' => 'id',
             'foreign' => 'fac'));

        $this->hasMany('CopisimPoste', array(
             'local' => 'id',
             'foreign' => 'filiere'));

        $this->hasMany('CopisimChoix', array(
             'local' => 'id',
             'foreign' => 'complement'));

        $this->hasMany('CopisimFlux', array(
             'local' => 'id',
             'foreign' => 'complement'));
    }
}